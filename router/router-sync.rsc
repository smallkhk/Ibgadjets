...
#
# The comment is "ibg:<subscription id>", not a bare "ibg". The id is
# what tells us a customer has started a NEW bundle, because RouterOS
# byte counters belong to the hotspot user and carry straight across from
# one bundle to the next. Without resetting them, a customer who used 4GB
# and then topped up got a 5GB bundle with 1GB on it.
#
# markRe matches both shapes so accounts created by older versions of
# this script, commented plain "ibg", are still recognised as ours.
:local mark "ibg"
:local markRe "^ibg"
...
#
# A genuinely wedged fetch would otherwise hold the lock forever and sync
# would stop for good, silently. So the holder is given five minutes and
# then assumed dead.
#
# Five MINUTES, not five refusals. Counting refusals looks equivalent and
# is not: run the script by hand a few times while the scheduler is also
# firing and the count reaches five in seconds, so the lock gets taken
# from a cycle that is very much alive. That produces exactly the
# concurrent cycles this lock exists to prevent, and concurrent cycles on
# a 128MB router are what caused the out-of-memory reboots.
#
# Uptime is the clock because it always moves forward at the same rate
# and needs no wall-clock time. Both globals vanish on reboot, which is
# correct: after a reboot no cycle is running.
# ---------------------------------------------------------------------
:global ibgBusy
:global ibgBusyAt

:local ibgNow [/system resource get uptime]

:if ($ibgBusy = true) do={
    :if ([:typeof $ibgBusyAt] = "time" and ($ibgNow - $ibgBusyAt) < 5m) do={
        :log warning "ibg-sync: previous cycle still running, skipping"
        :error "busy"
    }
    :log warning "ibg-sync: previous cycle wedged for over 5m, taking the lock"
}

:set ibgBusy true
:set ibgBusyAt $ibgNow
...
:do {
    ...
    :foreach p in=$profiles do={
        ...
        :if ([:len $pfound] = 0) do={
            ...
            # Only write when something actually differs. Rewriting
            # identical values every 60 seconds is thousands of config
            # writes a day onto 16MB of NAND, for no change at all.
            :local pid [:pick $pfound 0]
            :if ([/ip hotspot user profile get $pid rate-limit] != $prate or \
                 [/ip hotspot user profile get $pid shared-users] != $pshare) do={
                /ip hotspot user profile set $pid \
                    rate-limit=$prate shared-users=$pshare
            }
        }
    }
    ...
    :foreach u in=$users do={
        ...
        # The site sends what is LEFT. RouterOS measures what has been
        # USED. Writing "left" straight into limit-bytes-total counts the
        # same megabyte twice: the cap falls by 1MB on every poll while
        # the router's own counter climbs by 1MB, and a 500MB customer is
        # cut off at around 250MB.
        #
        # So the cap is what they have already spent plus what remains.
        # It stays correct as usage accrues, and it survives the counter
        # being reset — a reset makes used 0 and the cap becomes exactly
        # the remaining figure, which is right.
        :if ($bytes > 0 and [:len $found] > 0) do={
            :local fid [:pick $found 0]
            :local used ([/ip hotspot user get $fid bytes-in] + [/ip hotspot user get $fid bytes-out])
            :set bytes ($bytes + $used)
        }

        # Which bundle this account is on. A change here means a new
        ...
        :if ([:len $found] = 0) do={
            /ip hotspot user add name=$uname password=$pass \
                profile=$uprof \
                limit-bytes-total=$bytes comment=$umark
            :log info ("ibg-sync: provisioned " . $uname)
        } else={
            # Same reasoning as the profiles above: write only on a real
            # difference. With the cap computed as used+remaining it
            # barely moves between polls, so a settled customer costs no
            # writes at all.
            :local fid [:pick $found 0]
            ...
            :if ([/ip hotspot user get $fid limit-bytes-total] != $bytes or \
                 [/ip hotspot user get $fid profile] != $uprof or \
                 [/ip hotspot user get $fid password] != $pass or \
                 [/ip hotspot user get $fid comment] != $umark or \
                 [/ip hotspot user get $fid disabled] = true) do={
                /ip hotspot user set $fid password=$pass \
                    profile=$uprof \
    ...
    # -----------------------------------------------------------------
    :foreach h in=[/ip hotspot user find where comment~$markRe] do={
